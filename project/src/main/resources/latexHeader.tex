...
\newcommand{\rec}[3]{%
  \pgfsetlinewidth{1.4pt}%
  \newdimen\a\newdimen\b%
  \pgf@process{\pgfpointdiff{\pgfpointanchor{#3}{center}}{\pgfpointanchor{#2}{center}}}%
  \a=\pgf@x%
  \pgf@process{\pgfpointanchor{#2}{center}}%
  \pgfmathsetlength\a{\pgf@x-\a/2}
  \b=\pgf@y%
  \pgf@process{\pgfpointanchor{#1}{center}}%
  \pgfmathsetlength\a{(3*\pgf@x+2*\a)/5-3pt}
  \pgfmathsetlength\b{(3*\pgf@y+2*\b)/5+3pt}
  \pgfpathrectangle{\pgfpoint{\a}{\b}}{\pgfpoint{6pt}{-6pt}}%
  \pgfusepath{draw}%
}
...
\def\oreC{\rec{node@\forestove{id}}{node@\forestove{@first}}%
  {node@\forestOve{\forestove{@last}}{@previous}}}
\def\oreN{\rec{node@\forestove{id}}%
  {node@\forestove{@first}}{node@\forestove{@last}}}
